...
- **`osfdata` is published on production PyPI — `pip install osfdata`** (version **1.1.0**, 2026-07-29, <https://pypi.org/project/osfdata/>). The Python bindings move off the test index they had been pre-released on since `0.1.0`; the release ships the four abi3 wheels (Linux x86_64 + aarch64, macOS arm64, Windows x64) plus the sdist, published from `release.yml` via Trusted Publishing. Version 1.0.0 was prepared on 2026-07-25 but never tagged or uploaded, so the production line starts at 1.1.0 — the package CHANGELOG marks that entry accordingly. Repository documentation was updated in the same pass: the root `README.md` badge, table row and Quick Start, `STATUS.md`, and `implementations/python/{BUILD,RELEASE,CHANGELOG}.md` no longer reference the test index, and `BUILD.md` additionally lost a stale claim that the publish job was disabled by an `if: false` gate.
