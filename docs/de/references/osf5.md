...
| `string`   | variabel | UTF-8 kodiert, Länge durch Blockgröße definiert. OSF5-Schreiber speichern die Payload ohne abschließendes Nullbyte; OSF5-Leser dürfen kein abschließendes Byte entfernen – siehe [`osf_general.md`](../osf_general.md#hinweis-zur-nullterminierung-von-string-und-binary). |
| `binary` *(Alias: `bytearray`)* | variabel | Beliebige Bytefolgen für Bild-, Audio- oder andere Binärdaten mit MIME-Type. Maximale Länge wird durch `sizeoflengthvalue` bestimmt. OSF5-Schreiber speichern die Payload ohne abschließendes Nullbyte; OSF5-Leser dürfen kein abschließendes Byte entfernen – siehe [`osf_general.md`](../osf_general.md#hinweis-zur-nullterminierung-von-string-und-binary). |
...
- **Schreiber DÜRFEN KEIN** abschließendes Nullbyte (`0x00`) anhängen. Die Payload endet am letzten Datenbyte; `sizeoflengthvalue` definiert die exakte Länge.
