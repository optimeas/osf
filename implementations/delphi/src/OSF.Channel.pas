...
type
  ...
  public
    constructor Create(AIndex: Integer; const AName: string; AChannelType: TOSFChannelType; ADataType: TOSFDataType);
...
begin
  ...
  if Assigned(Val) then
    Result := StrToInt64Def(Val.Value, Default)
  else
    Result := Default;
end;
...
constructor TOSFChannelDef.Create(AIndex: Integer; const AName: string; AChannelType: TOSFChannelType; ADataType: TOSFDataType);
begin
  inherited Create;
  FIndex := AIndex;
  FName := AName;
...
var
  Ch: TOSFChannelDef;
begin
  Ch := TOSFChannelDef.Create(XMLAttrInt(Node, 'index', 0), XMLAttrStr(Node, 'name', ''),
    OSFChannelTypeFromString(XMLAttrStr(Node, 'channeltype', 'scalar')), OSFDataTypeFromString(XMLAttrStr(Node, 'datatype', 'double')));
  try
    ...
    Ch.LengthFieldSize := OSFLengthFieldSizeFromInt(XMLAttrInt(Node, 'sizeoflengthvalue', OSF_DEFAULT_LENGTH_FIELD_SIZE));
    ...
    Ch.Comment := XMLAttrStr(Node, 'comment', XMLAttrStr(Node, 'description', ''));
...
var
  Ch: TOSFChannelDef;
begin
  Ch := TOSFChannelDef.Create(JSONInt(Obj, 'index', 0), JSONStr(Obj, 'name', ''), OSFChannelTypeFromString(JSONStr(Obj, 'channeltype', 'scalar')),
  ...
  try
    ...
    Ch.LengthFieldSize := OSFLengthFieldSizeFromInt(JSONInt(Obj, 'sizeoflengthvalue', OSF_DEFAULT_LENGTH_FIELD_SIZE));
    ...
    Ch.Comment := JSONStr(Obj, 'comment', JSONStr(Obj, 'description', ''));
