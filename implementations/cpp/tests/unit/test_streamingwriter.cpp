...
struct WrittenFile {
    ...
    std::string metablock_json;      // exactly metablockLen bytes
...
WrittenFile read_written_file(std::filesystem::path const& path) {
    ...
    wf.metablock_json.assign(mb.data(), metablockLen);
...
TEST(StreamingWriterLifecycle, add_channel_after_start_returns_error) {
    TempFileGuard g{make_temp_path()};
    osf::StreamingWriter w{g.path};
    ASSERT_TRUE(w.addChannel(make_double_scalar_channel("a")).has_value());
    ASSERT_TRUE(w.start().has_value());
    auto r = w.addChannel(make_double_scalar_channel("b"));
    ASSERT_FALSE(r.has_value());
    EXPECT_EQ(r.error().code, osf::Error::Code::InvalidArgument);
}
...
TEST(StreamingWriterPreWrite, add_channel_returns_sequential_indices) {
    TempFileGuard g{make_temp_path()};
    osf::StreamingWriter w{g.path};
    auto r0 = w.addChannel(make_double_scalar_channel("a"));
    auto r1 = w.addChannel(make_double_scalar_channel("b"));
    auto r2 = w.addChannel(make_double_scalar_channel("c"));
...
TEST(StreamingWriterByteExact, metablock_json_parses_with_expected_content) {
    ...
    osf::StreamingWriter w{g.path};
    w.setCreator("test:1");
    ASSERT_TRUE(w.addChannel(make_double_scalar_channel("Sensor/T")).has_value());
    ASSERT_TRUE(w.start().has_value());
    ASSERT_TRUE(w.close().has_value());

    auto const wf = read_written_file(g.path);
    auto parsed = osf::parseMetablockJson(wf.metablock_json);
...
TEST(StreamingWriterByteExact, first_bcStartData_block_layout_for_double) {
    TempFileGuard g{make_temp_path()};
    osf::StreamingWriter w{g.path};
    ASSERT_TRUE(w.addChannel(make_double_equidistant_channel("a")).has_value());
    ASSERT_TRUE(w.start().has_value());
...
TEST(StreamingWriterRoundtrip,
    ...
    {
        osf::StreamingWriter w{g.path};
        ASSERT_TRUE(w.addChannel(make_double_equidistant_channel("eq")).has_value());
        ASSERT_TRUE(w.start().has_value());
...
TEST(StreamingWriterRoundtrip, timestamped_bool_roundtrips) {
    ...
    auto const pairs = osf::asBoolsFlat(*ts_ch);
...
TEST(StreamingWriterRoundtrip, timestamped_uint16_roundtrips) {
    ...
    auto const* ts_ch = std::get_if<osf::TimestampedChannel>(
        mgr->channel("u16"));
    ASSERT_NE(ts_ch, nullptr);
    auto const pairs = osf::asUint16Flat(*ts_ch);
    ASSERT_TRUE(pairs.has_value());
...
TEST(StreamingWriterRoundtrip,
    ...
    auto const* ts_ch = std::get_if<osf::TimestampedChannel>(
        mgr->channel("i32"));
    ASSERT_NE(ts_ch, nullptr);
    auto const pairs = osf::asInt32Flat(*ts_ch);
    ASSERT_TRUE(pairs.has_value());
...
TEST(StreamingWriterRoundtrip, timestamped_uint64_roundtrips) {
    ...
    auto const* ts_ch = std::get_if<osf::TimestampedChannel>(
        mgr->channel("u64"));
    ASSERT_NE(ts_ch, nullptr);
    auto const pairs = osf::asUint64Flat(*ts_ch);
    ASSERT_TRUE(pairs.has_value());
...
TEST(StreamingWriterRoundtrip,
    ...
    auto const* ts_ch = std::get_if<osf::TimestampedChannel>(
        mgr->channel("d"));
    ASSERT_NE(ts_ch, nullptr);
    auto const pairs = osf::asDoublesFlat(*ts_ch);
    ASSERT_TRUE(pairs.has_value());
    ASSERT_EQ(pairs->size(), 100'000u);
...
TEST(StreamingWriterRoundtrip, timestamped_gps_array_roundtrips) {
    ...
    auto const* ts_ch = std::get_if<osf::TimestampedChannel>(
        mgr->channel("gps"));
    ASSERT_NE(ts_ch, nullptr);
    auto const pairs = osf::asGpsFlat(*ts_ch);
    ASSERT_TRUE(pairs.has_value());
    ASSERT_EQ(pairs->size(), 3u);
...
TEST(StreamingWriterPreWrite,
    ...
    auto r = w.writeTimestampedBinary(
        0, 0LL, osf::BinarySample::fromVector(too_big));
    ASSERT_FALSE(r.has_value());
    EXPECT_EQ(r.error().code, osf::Error::Code::InvalidBlock);
    EXPECT_NE(r.error().message.find("65526 bytes"), std::string::npos);
}
...
TEST(StreamingWriterLifecycle,
    ...
    TempFileGuard g{make_temp_path()};
    {
        ...
        ASSERT_TRUE(w1.addChannel(make_double_scalar_channel("mv")).has_value());
...
TEST(StreamingWriterLifecycle, self_move_assignment_is_safe) {
    ...
    TempFileGuard g{make_temp_path()};
    osf::StreamingWriter w{g.path};
    ASSERT_TRUE(w.addChannel(make_double_scalar_channel("sm")).has_value());
    ASSERT_TRUE(w.start().has_value());
