...
Last updated: 2026-06-03 (after C++ Phase 9 — the opt-in throwing
convenience layer; next is Phase 10, CI integration).
...
1. **C++ implementation** — a phased plan in DECISIONS.md §20. Driven by
   focused per-phase sessions. Phases 1–9 done (both OSF5 writers, the
   optional `StaleValueGuard`, transparent OSFZ read, the opt-in throwing
   layer); next up: **Phase 10** (CI integration).
...
2026-06-03), and **9 (throwing convenience layer, completed
2026-06-03)** all done. Both OSF5 writer classes, the optional
freshness layer, transparent gzip/zlib decompression on read, and the
opt-in throwing layer are in place. **304/304 ctest green** (0 warnings
under `/W4 /permissive-`).
...
- **Phase 10:** CI integration — extend `.github/workflows/ci.yml` to
  cover `implementations/cpp/**` (path filter currently excludes it) with
  a Linux/macOS/Windows job matrix (configure + build + ctest; the
  FetchContent zlib/gtest deps fetch cleanly on runners with working
  HTTPS). Then Phase 11 (C ABI wrapper).
...
3. If continuing the **C++ track**: read DECISIONS §20 + §6 + §7, run
   ...
   overrides — see the network caveat below). Phases 1–9 are done (both
   OSF5 writers, the optional `StaleValueGuard`, transparent OSFZ read,
   the opt-in throwing layer); next is Phase 10 (CI integration).
