...
Last updated: 2026-06-03 (after C++ Phase 10 — CI integration; next is
Phase 11, the C ABI wrapper — the last C++ phase).
...
1. **C++ implementation** — a phased plan in DECISIONS.md §20. Driven by
   focused per-phase sessions. Phases 1–10 done (both OSF5 writers, the
   optional `StaleValueGuard`, transparent OSFZ read, the opt-in throwing
   layer, CI on Linux/macOS/Windows); next up: **Phase 11** (C ABI
   wrapper — the last phase).
...
## C++ track — Phase 11 is next (the last phase)
...
2026-06-03), 9 (throwing convenience layer, 2026-06-03), and **10
(CI integration, completed 2026-06-03)** all done. Both OSF5 writer
classes, the optional freshness layer, transparent gzip/zlib
decompression on read, and the opt-in throwing layer are in place, and
CI builds + tests them on Linux/macOS/Windows with warnings-as-errors.
**304/304 ctest green** on every leg (0 warnings under `/W4 /permissive-`
locally; `/WX` / `-Werror` in CI).
...
- **Phase 11 (last phase):** C ABI wrapper — a C-callable shared-library
  surface (Windows DLL / ActiveX-OCX, future-binding foundation) behind
  the existing `OSF_BUILD_C_API` option (default OFF). Write a DECISIONS
  entry first (opaque handle types, C error codes mirroring `Error::Code`,
  string ownership, ABI stability), then a `c_api/` subtree wrapping
  `DataManager` / the writers, with CI building it on the matrix.
...
3. If continuing the **C++ track**: read DECISIONS §20 + §6 + §7, run
   the C++ build flow (304/304 baseline; configure with the local
   `FETCHCONTENT_SOURCE_DIR_ZLIB` + `FETCHCONTENT_SOURCE_DIR_GOOGLETEST`
   overrides — see the network caveat below). Phases 1–10 are done (both
   OSF5 writers, the optional `StaleValueGuard`, transparent OSFZ read,
   the opt-in throwing layer, CI on Linux/macOS/Windows); next is Phase 11
   (C ABI wrapper, the last phase). CI now runs on C++ pushes — verify
